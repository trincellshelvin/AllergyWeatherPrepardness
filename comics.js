const url = 'https://opentdb.com/api.php?amount=30&category=29';
const ONE_DAY = 24 * 60 * 60 * 1000; // 24 hours in milliseconds

let currentQuestionIndex = 0;
let score = 0;

async function getComicsTriviaQuestions() {
    try {
        let response = await fetch(url);
        let data = await response.json();
        localStorage.setItem('ComicsTriviaQuestions', JSON.stringify(data.results));
        localStorage.setItem('ComicsFetchTimestamp', Date.now());
        displayCurrentQuestion();
    } catch (error) {
        console.error('Error fetching Comics trivia questions:', error);
    }
}

function displayCurrentQuestion() {
    const questions = JSON.parse(localStorage.getItem('ComicsTriviaQuestions'));
    const triviaDiv = document.getElementById('comicstrivia-container');
...
function loadComicsQuestionsFromLocalStorage() {
    const storedQuestions = localStorage.getItem('ComicsTriviaQuestions');
    const fetchTimestamp = localStorage.getItem('ComicsFetchTimestamp');
    ...
    getComicsTriviaQuestions();
}

document.getElementById('getQuestions').addEventListener('click', getComicsTriviaQuestions);

loadComicsQuestionsFromLocalStorage();
