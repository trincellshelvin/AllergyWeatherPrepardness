const url = 'https://opentdb.com/api.php?amount=30&category=31';
const ONE_DAY = 24 * 60 * 60 * 1000; // 24 hours in milliseconds

let currentQuestionIndex = 0;
let score = 0;

async function getJapaneseTriviaQuestions() {
    try {
        let response = await fetch(url);
        let data = await response.json();
        localStorage.setItem('JapaneseTriviaQuestions', JSON.stringify(data.results));
        localStorage.setItem('JapaneseFetchTimestamp', Date.now());
        displayCurrentQuestion();
    } catch (error) {
        console.error('Error fetching Japanese Anime trivia questions:', error);
    }
}

function displayCurrentQuestion() {
    const questions = JSON.parse(localStorage.getItem('JapaneseTriviaQuestions'));
    const triviaDiv = document.getElementById('japanesetrivia-container');
...
function loadJapaneseQuestionsFromLocalStorage() {
    const storedQuestions = localStorage.getItem('JapaneseTriviaQuestions');
    const fetchTimestamp = localStorage.getItem('JapaneseFetchTimestamp');
    ...
    getJapaneseTriviaQuestions();
}

document.getElementById('getQuestions').addEventListener('click', getJapaneseTriviaQuestions);

loadJapaneseQuestionsFromLocalStorage();
